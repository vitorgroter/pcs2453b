...
InformationManager::InformationManager(Scheduler * s, std::string filename) : scheduler(s) {
	...
	if (!file.is_open())
		throw std::runtime_error("Could not read filesystem info: " + filename + " does not exist.");
	...
	while (!file.eof()) {
		...
		if (isPublic < 0 || isPublic > 1)
			throw std::runtime_error("Invalid information in filesystem description: public attribute should be 0 or 1.");
		...
		if (fileSize < 0)
			throw std::runtime_error("Invalid information in filesystem description: filesize must be positive.");

		fullpath = readQuotes(lineStream);
		if (!isPath(fullpath))
			throw std::runtime_error("Invalid information in filesystem description: invalid path.");

		std::string strParentDir = getParentDir(fullpath);
		Directory * parentDir = root->recursivelyCreateDir("." + strParentDir); 
		File * file = new PlainFile(fullpath.substr(strParentDir.length(), fullpath.size() - strParentDir.length()), fileSize);
